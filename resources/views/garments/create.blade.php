            ...
            <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-xl sm:rounded-2xl border dark:border-gray-700">
                <div class="p-6 text-gray-900 dark:text-gray-200">
                    <form method="POST" action="{{ route('garments.store') }}" enctype="multipart/form-data">
                        @csrf
                        <h3
                            class="text-xl font-semibold mb-6 text-indigo-600 dark:text-indigo-400 border-b dark:border-gray-700 pb-2">
                            Datos del Lote de Prendas
                        </h3>
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            <div class="md:col-span-1">
                                <label class="block text-sm font-semibold mb-1">PV (Código)</label>
                                <input id="pv" type="text" name="pv" value="{{ old('pv', $randomPV) }}"
                            ...
                            <div class="md:col-span-1">
                                <label class="block text-sm font-semibold mb-1">Tallas
                                    del Lote</label>
                                <input id="sizes" type="text" name="sizes"
                                    value="{{ old('sizes') }}" required
                                    placeholder="Ej: S, M, L, XL / 30, 32, 34"
                                    class="w-full rounded-lg bg-gray-50 dark:bg-gray-800 border-gray-300 dark:border-gray-600 focus:ring-indigo-500 focus:border-indigo-500">
                                @error('sizes')
                                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold mb-1">Cantidad de Prendas (Lote
                                    Completo)</label>
                                <input id="quantity_in" type="number" name="quantity_in"
                                    value="{{ old('quantity_in', 1) }}" required min="1"
                                    class="w-full rounded-lg bg-gray-50 dark:bg-gray-800 border-gray-300 dark:border-gray-600 focus:ring-indigo-500 focus:border-indigo-500">
                                @error('quantity_in')
                                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                                ...
                                <select name="stitching_line_id" required
                                    ...
                                    @foreach ($lines as $line)
                                        <option value="{{ $line->id }}"
                                            {{ old('stitching_line_id') == $line->id ? 'selected' : '' }}>
                                            {{ $line->name }}
                                            {{ $line->is_external_service ? '(Servicio Externo)' : '' }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('stitching_line_id')
                                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <select name="motive_id" required
                                    ...
                                    @foreach ($motives as $motive)
                                        <option value="{{ $motive->id }}"
                                            {{ old('motive_id') == $motive->id ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Sube una imagen (JPG, PNG) para
                                    documentar el problema.</p>
                                @error('defect_photo')
                                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="audit_level" required
                                    class="w-full rounded-lg bg-gray-50 dark:bg-gray-800 border-gray-300 dark:border-gray-600 focus:ring-indigo-500 focus:border-indigo-500">
                                    <option value="normal" {{ old('audit_level') == 'normal' ? 'selected' : '' }}>
                                        Normal (Atención Estándar)</option>
                                    <option value="urgente" {{ old('audit_level') == 'urgente' ? 'selected' : '' }}>
                                        Urgente (Requiere Auditoría Rápida)</option>
                                </select>
                                @error('audit_level')
                                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                @enderror
