{{-- resources/views/motives/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Editar Motivo: ') }}
            <span class="text-indigo-600 dark:text-indigo-400">{{ $motive->name }}</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-xl sm:rounded-2xl border dark:border-gray-700">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('motives.update', $motive) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            >
                                ...
                                @foreach($motiveTypes as $key => $value)
                                    <option value="{{ $key }}" {{ old('type', $motive->type) == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center justify-end mt-8 pt-4 border-t dark:border-gray-700">
                            ...
                            <button
                                type="submit"
                                class="px-6 py-2 bg-green-600 text-white font-semibold rounded-xl
